  ...
  return (
    <div className="h-full w-full bg-[#050505] overflow-y-auto pb-40 scrollbar-hide">
      <header className="p-8 pt-16 md:p-12">
        ...
        <p className="text-zinc-500 text-xs font-bold uppercase tracking-[0.4em]">Apple Vertical Originals</p>
      </header>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[1500] animate-apple p-8 overflow-y-auto">
           ...
           <div className="max-w-4xl mx-auto pt-20">
              <div className="flex flex-col md:flex-row gap-12 mb-16">
                 <img src={selectedSeries.cover_image} className="w-64 aspect-[9/16] rounded-[2.5rem] object-cover shadow-2xl" />
                 <div className="flex-1">
                    ...
                    <button className="px-12 py-5 bg-white text-black font-black rounded-2xl hover:bg-zinc-200 transition-all">SEGUIR SÉRIE</button>
                 </div>
              </div>
              <div className="space-y-4">
                 {content.episodes.map(ep => (
                   <div key={ep.id} onClick={() => onOpen(ep, selectedSeries)} className="p-6 bg-white/5 border border-white/5 rounded-3xl flex items-center gap-6 cursor-pointer hover:bg-white/10 transition-all">
                      ...
                      <div>
                         <span className="text-rose-500 font-black text-[10px] uppercase tracking-widest">Episódio {ep.episode_number}</span>
